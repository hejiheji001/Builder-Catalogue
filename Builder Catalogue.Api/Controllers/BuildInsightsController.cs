...
[ApiController]
[Route("insights")]
public class BuildInsightsController(LEGOSetService setsService) : ControllerBase
{
    ...
    {
        var result = await setsService.GetBuildableSetsAsync(username);
        return Ok(result);
    }
    ...
    {
        var result = await setsService.FindCollaboratorsAsync(username, setName);
        return Ok(result);
    }
    ...
    {
        var result = await setsService.GetBuildSizeRecommendationAsync(username, percentile);
        return Ok(result);
    }
    ...
    {
        var result = await setsService.GetColorFlexibilityAsync(username);
        return Ok(result);
    }
}
